//
//  ProgramSelector.swift
//  Lores_Example
//
//  Created by Wolf McNally on 3/3/18.
//  Copyright © 2018 CocoaPods. All rights reserved.
//

//let program = EmptyProgram()
//let program = HelloPixelProgram()
//let program = DotsAndLinesProgram()
//let program = RectanglesProgram()
//let program = PebblesProgram()
//let program = GravelProgram()
//let program = MoveProgram()
//let program = MandalaProgram()
//let program = MushroomProgram()
//let program = BallProgram()
//let program = SparklerProgram()
//let program = WhirlwindProgram()
//let program = DripProgram()
//let program = RainingHeartsProgram()
//let program = SargonChessboardProgram()
//let program = SwiftLesson1Page1()
//let program = SwiftLesson1Page2()
//let program = SwiftLesson1Page3()
let program = SnakeProgram()
