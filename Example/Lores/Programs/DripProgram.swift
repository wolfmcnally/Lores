...
class DripProgram: Program {
    ...
    private struct Drip {
        ...
        func isEmpty(at offset: Offset) -> Bool {
            let pos = position + offset
            guard backgroundCanvas.isValidPoint(pos) else { return false }
...
//            }
    ...
    private func drawBasin() {
        ...
        backgroundCanvas[bounds.minX, bounds.minY ... bounds.maxY] = basinColor
        backgroundCanvas[bounds.maxX, bounds.minY ... bounds.maxY] = basinColor
        backgroundCanvas[bounds.minX ... bounds.maxX, bounds.maxY] = basinColor
    }
    ...
    private func drawObstacles() {
        ...
        for y in (bounds.minY + 5) ... (bounds.maxY - 1) {
            for x in (bounds.minX + 1) ... (bounds.maxX - 1) {
                if Random.number() <= percentObstacles {
    ...
    private func addDrip() {
        guard let startPosition = centerStartPosition() else { return }
